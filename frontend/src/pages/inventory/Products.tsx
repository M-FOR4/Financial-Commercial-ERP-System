    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[calc(100vh-4rem)] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-border sticky top-0 bg-card z-10 relative">
          ...
          <h3 className="text-lg font-bold text-foreground pl-12">{isEditing ? 'تعديل الصنف' : 'إضافة صنف جديد'}</h3>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">إدارة أصناف المخزون — {products.length} صنف{lowStockCount > 0 && <span className="mr-2 px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/30">{lowStockCount} مخزون منخفض</span>}</p>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl p-4 flex flex-wrap gap-4 items-center">
        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="بحث بالرمز أو الاسم..." className="flex-1 min-w-[200px] px-4 py-2 bg-input border-border rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring text-sm" />
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="px-4 py-2 bg-input border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-ring text-sm">
          <option value="">جميع الفئات</option>{categories.filter(c => c.isActive).map(c => <option key={c.id} value={c.id}>{c.code} — {c.name}</option>)}
        </select>
      </div>
      {error && <div className="p-4 bg-destructive/10 border border-destructive/30 rounded-lg text-destructive text-sm">فشل في تحميل الأصناف.</div>}
      {isLoading && <div className="flex items-center justify-center p-12 text-muted-foreground space-x-3"><div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin" /><span>جاري تحميل الأصناف...</span></div>}
      {!isLoading && !error && (
              ...
              <tbody className="divide-y divide-border/50">
                {products.length === 0 ? <tr><td colSpan={9} className="px-5 py-12 text-center text-muted-foreground">لم يتم العثور على أصناف.</td></tr> : products.map(p => (
                  <tr key={p.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-5 py-3 font-semibold text-primary text-right">{p.sku}</td>
                    <td className="px-5 py-3 text-foreground text-right">{p.name}</td>
                    <td className="px-5 py-3 text-muted-foreground text-right">{p.categoryName}</td>
                    <td className="px-5 py-3 text-center text-muted-foreground">{uomAr[p.unitOfMeasure] || p.unitOfMeasure}</td>
                    <td className="px-5 py-3 text-left text-foreground">{formatCurrency(p.purchasePrice)}</td>
                    <td className="px-5 py-3 text-left text-foreground">{formatCurrency(p.sellingPrice)}</td>
                    <td className="px-5 py-3 text-left">
                      <span className={`font-semibold ${p.isLowStock ? 'text-amber-400' : 'text-emerald-400'}`}>{formatStock(p.currentStock)}</span>
                      {p.isLowStock && <span className="mr-1.5 px-1.5 py-0.5 text-[9px] font-bold rounded bg-amber-500/20 text-amber-400 border border-amber-500/30">منخفض</span>}
                    </td>
                    <td className="px-5 py-3 text-center">
                      <span className={`px-2 py-0.5 text-[10px] font-semibold rounded-full ${p.isActive ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30' : 'bg-destructive/15 text-destructive border border-destructive/30'}`}>{p.isActive ? 'نشط' : 'غير نشط'}</span>
                    </td>
                    <td className="px-5 py-3 text-center">
                      <button onClick={() => { setEditingProduct(p); setShowModal(true); }} className="px-2.5 py-1 text-xs text-primary hover:text-primary/80 bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-colors">تعديل</button>
                    </td>
                  </tr>
                ))}
              </tbody>
